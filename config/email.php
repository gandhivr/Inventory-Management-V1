 ...
 * Handles sending email notifications for orders and other system events
 */

// Email configuration - UPDATE THESE WITH YOUR GMAIL CREDENTIALS!
define('SMTP_HOST', 'smtp.gmail.com');
define('SMTP_PORT', 587);
define('SMTP_USERNAME', 'vrajgandhi06@gmail.com'); // UPDATE: Your Gmail address
define('SMTP_PASSWORD', 'your-app-password-here'); // UPDATE: Your Gmail App Password
define('SMTP_FROM_EMAIL', 'vrajgandhi06@gmail.com'); // UPDATE: Your Gmail address
define('SMTP_FROM_NAME', 'Inventory Management System');

/**
 * Send email using PHP mail() function
 * For production, consider using PHPMailer or similar library for better reliability
 * 
...
function sendEmail($to, $subject, $message, $headers = []) {
    ...
    $is_localhost = (isset($_SERVER['SERVER_NAME']) && 
    ...
    try {
        // Try PHP mail() first (works on some hosts)
        ...
        $error_log = $log_dir . '/email_errors.log';
        file_put_contents($error_log, date('Y-m-d H:i:s') . " - Failed to send to $to: " . $e->getMessage() . "\n", FILE_APPEND);
        return false;
    }
}
 ...
 * Fallback method when mail() doesn't work (like on InfinityFree)
 */
function sendViaSMTP($to, $subject, $message) {
    try {
        ...
        if (strpos($response, '535') !== false || strpos($response, '5.7.8') !== false) {
            fclose($smtp);
            error_log("SMTP authentication failed. Check your Gmail credentials and app password.");
            return false;
        }
 ...
 * @param array $order_details Order details including buyer info, items, total
 * @return bool Success status
 */
 ...
 * @param array $supplier_items Items from this supplier
 * @param array $order_details Order details including buyer info
 * @return bool Success status
 */
function sendOrderNotificationToSupplier($order_id, $supplier_id, $supplier_items, $order_details) {
    ...
    try {
        ...
        // Build order items HTML for this supplier
        $items_html = '';
        $supplier_total = 0;
 ...
 * 
 * @param int $order_id Order ID
 * @param array $order_details Order details including items and total
 * @return bool Success status
 */
